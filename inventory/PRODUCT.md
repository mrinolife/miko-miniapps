...
## Tonight Mode
Tonight Mode is the smallest honest dinner-decision slice: the user picks exactly one capacity state (`Open + eat`, `Low effort`, or `I can cook`) and sees exactly three ranked recipes from their local recipe box, with starter fallbacks only when the box is sparse. Choosing a meal creates one sanitized `body-cook-plan-v1` record containing only recipe id/title, capacity, chosen route, status, step index, blocker ingredients `{name,state,source}`, confirmations, and pause state. The plan never copies source media IDs, `local_reference_id`, private notes, transcripts, blobs, or whole recipe objects.

Before cooking, the user confirms only uncertain and missing blockers with three tap states: `on_hand`, `uncertain`, or `missing`. Those confirmations persist into ranking, explanation copy, and Visual Cook. Both `Cook as planned` and `Make it easier` open the same recipe-specific Visual Cook path. Mid-cook route changes update only the remaining actions, tool count, and cleanup expectation while retaining completed progress. If the recipe memory is incomplete, Visual Cook falls back to an explicitly labeled minimal checklist and never invents temperatures or doneness.

The cook plan is interruption-safe: route, step, status, confirmations, and pause or safe-exit state all survive reloads. `Cannot finish` supports honest exits such as saving components, refrigerating where appropriate, switching to a fallback meal, or abandoning the plan, without auto-mutating inventory or claiming food was consumed. Completion memory records actual effort, route used, cleanup light/medium/heavy, low-capacity fit, and an optional note in `body-tonight-mode-v1` history; those richer memories feed the next local ranking pass.
